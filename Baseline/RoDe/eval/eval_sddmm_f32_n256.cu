...
__global__ void StandKernel(
    int m, int n, int k, const int *__restrict__ row_indices,
    const int *__restrict__ row_offsets, const int *__restrict__ column_indices,
    const float *__restrict__ values, const float *__restrict__ lhs_matrix,
    const float *__restrict__ rhs_matrix, float *__restrict__ out) {
...
void StandCall(int m, int n, int k, const int *__restrict__ row_indices,
               const int *__restrict__ row_offsets,
               const int *__restrict__ column_indices,
               const float *__restrict__ values,
               const float *__restrict__ lhs_matrix,
               const float *__restrict__ rhs_matrix, float *__restrict__ out) {
  StandKernel<<<(m + 31) / 32, 32>>>(m, n, k, row_indices, row_offsets,
                                     column_indices, values, lhs_matrix,
                                     rhs_matrix, out);
}
...
int main(int argc, char **argv) {
...
#ifdef VALIDATE
  StandCall(m, n, k, c_sm.RowIndices(), c_sm.RowOffsets(), c_sm.ColumnIndices(),
            c_sm.Values(), d_B1.Values(), d_B2.Values(), d_C);
#endif
  ...
  //   SputnikSddmm(m, k, n, c_sm.Nonzeros(), c_sm.RowIndices(),
  //   c_sm.RowOffsets(),
  //                c_sm.ColumnIndices(), c_sm.Values(), d_B1.Values(),
  //                d_B2.Values(), d_C1, stream1);
  ...
  for (int i = 0; i < ITER; ++i)
    RoDeSDDMM_n256(c_sm.n_segs, c_sm.n_segs_residue, n, k, c_sm.seg_row_indices,
                   c_sm.seg_row_indices_residue, c_sm.seg_st_offsets,
